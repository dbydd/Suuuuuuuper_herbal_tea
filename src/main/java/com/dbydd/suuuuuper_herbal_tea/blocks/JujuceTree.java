...
public class JujuceTree extends BlockBase implements IGrowable {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (tier > 3) {
            ...
            if (tier > 4) {
                ItemHandlerHelper.giveItemToPlayer(player, new ItemStack(RegisteredItems.REDDATE, count));
            } else {
                ItemHandlerHelper.giveItemToPlayer(player, new ItemStack(RegisteredItems.GREENDATE, count));
            }
            worldIn.setBlockState(pos, state.with(GROW_TIER, state.get(GROW_TIER) - 1));
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
        if(!worldIn.isRemote()) {
            ItemEntity itemEntity = new ItemEntity(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(RegisteredBlocks.JUJUCE_TREE_TREE));
