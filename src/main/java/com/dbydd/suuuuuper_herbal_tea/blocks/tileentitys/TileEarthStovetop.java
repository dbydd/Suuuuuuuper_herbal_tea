...
import com.dbydd.suuuuuper_herbal_tea.items.BigBlackPotItem;
...
public class TileEarthStovetop extends TileEntity implements ITickableTileEntity {
    ...
    public TileEarthStovetop() {
        super(RegisteredTileEntities.TILE_EARTH_STOVETOP_TYPE.get());
    }
    ...
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
        if (cap == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY) {
            if (side == world.getBlockState(pos).get(EarthStovetop.FACING)) {
    ...
    public void onBlockActived(World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (heldItem.isEmpty()) {
                if (player.isSneaking()) {
                    if (hasBlackPot) {
                        ...
                        if (flag) {
                            ItemStack stack = new ItemStack(RegisteredItems.BIG_BLACK_POT_ITEM);
            ...
            } else if (heldItem.getItem() instanceof BigBlackPotItem) {
                if (!hasBlackPot) {
    ...
    private void finishCook() {
        ...
        for (int i = 0; i < slots; i++) {
            ...
            tank.setFluid(new FluidStack(RegisteredFluids.TEA_WATER.fluid.get().getFluid(), (int) waterAmount));
        }
