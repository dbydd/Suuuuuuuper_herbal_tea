...
public class EarthStovetop extends BlockBase {
    ...
    public VoxelShape getCollisionShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
        ...
        if(tileEntity != null) {
            return ((TileEarthStovetop)tileEntity).hasBlackPot()?shape_withpot:shape;
        }
        return shape;
    }
    ...
    public int getLightValue(BlockState state, IBlockReader world, BlockPos pos) {
        ...
        if(tileEntity instanceof TileEarthStovetop) {
            return ((TileEarthStovetop)tileEntity).isIsburning()?12:0;
        }
        return 0;
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, BlockState state, PlayerEntity player) {
        if(!worldIn.isRemote()) {
            ItemStackHandler fuel_ash_handler = ((TileEarthStovetop) worldIn.getTileEntity(pos)).getFuel_ash_Handler();
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new TileEarthStovetop();
    }

    @Override
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ((TileEarthStovetop) worldIn.getTileEntity(pos)).onBlockActived(worldIn, pos, player, handIn, hit);
