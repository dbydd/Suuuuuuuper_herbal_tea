...
public class WorldTeaTreeChunkGenerator extends ChunkGenerator<GenerationSettings> {
    ...
    public WorldTeaTreeChunkGenerator(IWorld worldIn, BiomeProvider biomeProviderIn) {
        super(worldIn, biomeProviderIn, new WorldTeaTreeGenerationSettings());
        generate_Map.put(0.1, RegisteredBlocks.WORLD_TEA_TREE_LEAVE_GLOW.getDefaultState());
        generate_Map.put(0.2, Blocks.OAK_WOOD.getDefaultState());
        generate_Map.put(0.6, RegisteredBlocks.WORLD_TEA_TREE_LEAVE.getDefaultState());
    }
                            ...
                            if (Math.sqrt(Math.pow(x, 2) + Math.pow(z, 2)) <= radius) {
                                ...
                                if (world.getBlockState(blockPos).getBlock() == Blocks.AIR) {
                                    ...
                                    if (RandomUtils.outputBooleanByChance(rand, 0.001)) {
                                        world.setBlockState(new BlockPos(blockPos.getX(), blockPos.getY() + 1, blockPos.getZ()), RegisteredBlocks.WORLD_TEA_TREE_FRUIT.getDefaultState(), 3);
                                    }
                                }
    ...
    public void makeBase(IWorld worldIn, IChunk chunkIn) {
        ...
        if (MathUtils.inCircle(chunkPos.x, chunkPos.z, MAIN_BRANCH_CHUNK_RANGE)) {
            ...
            if (chunkPos.x == 0 && chunkPos.z == 0 && !worldIn.isRemote()) {
                ...
                Template tea_pavilion = structureTemplateManager.getTemplate(new ResourceLocation(SuuuuuuuperHerbalTea.NAME, "tea_pavilion"));
